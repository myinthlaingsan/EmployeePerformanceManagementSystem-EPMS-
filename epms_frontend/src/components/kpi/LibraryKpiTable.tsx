        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white border-b-2 border-gray-800">
              <th className="px-3 py-2 text-sm font-bold text-gray-900 border-r border-gray-200 text-center w-1/3">KPI</th>
              <th className="px-3 py-2 text-sm font-bold text-gray-900 border-r border-gray-200 text-center w-1/5">Category</th>
          ...
          <tbody>
            {details.map((detail, index) => (
              <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 focus-within:bg-blue-50/20 transition-colors">
                ...
                <td className="border-r border-gray-200 p-0">
                  ...
                  >
                    {!isReadOnly && <option value={0}>Select Category...</option>}
                    {isReadOnly && detail.categoryName && <option value={detail.categoryId}>{detail.categoryName}</option>}
                    {categories.map(c => (
          ...
          <tfoot className="bg-gray-50 border-t-2 border-gray-800">
            <tr>
              ...
              <td className="px-3 py-2 text-right border-r border-gray-200">
                <span className={`text-sm font-bold ${totalWeight === 100 ? 'text-gray-900' : 'text-red-600'}`}>
                  {totalWeight}
                </span>
              </td>
              {!isReadOnly && <td></td>}
